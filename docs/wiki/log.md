...
will build on. No CLI, firmware-app, or baseline changes — pure host-testable
libraries, so existing modem baselines are untouched.

- **Architectural decision:** BPSK transmits real, but carrier-frequency offset
  (CFO), phase offset, and a Costas loop are rotations in the complex plane. So
  the sample path becomes complex **from the impairment channel onward** — a new
  `cq15_t` (I/Q int16, Q1.15). The TX path and RRC shaper stay real and
  bit-exact; complex appears only where the physics requires it. Control loops
  (PR 2) will be float; only the signal is q15.
- `lib/dsp/inc/complexq15.h` (header-only): `cq15_t` + `cq15_make/add/mul/conj/
  scale_real/dot_re/from_real/real`. `cq15_mul` uses a **64-bit** accumulator —
  the sum of two q30 products reaches 2^31 and overflows int32 (the `(-1-j)^2`
  corner, caught by a host test). Reuses `fixed.h` rounding/saturation.
- `lib/dsp/{inc/sincos.h,src/sincos.c}` + `src/sincos_lut.h` (generated by
  `src/gen_sincos_lut.py`): `dsp_sincos_q15(uint32_t phase, ...)` over a
  checked-in 1024-entry q15 table with linear interpolation (worst-case ~0.15
  LSB). Phase is full-circle uint32 (2^32 == 2*pi) so wrap is free/exact. A
  checked-in integer LUT is deterministic host vs target, unlike libm sinf/cosf.
  Plus float->phase config helpers (`nco_phase_from_cycles/_rad`).
- `lib/dsp/{inc/nco.h,src/nco.c}`: `nco_t` phasor generator — `nco_step` returns
  `e^{jθ}` as cq15 then advances phase; `nco_set_incr` retunes it (for the PR-2
  Costas loop).
- `lib/channel/{inc/impair.h,src/impair.c}`: `channel_impair_apply` injects
  fractional-sample timing offset (linear interp) → CFO rotate → phase offset on
  a real input block, producing cq15. Every field is "off" at zero/identity.
  Plus `channel_awgn_apply_cq15` in `awgn.c` — per-axis sigma on I and Q (total
  noise power doubles, but the in-phase projection after de-rotation recovers the
  same real-axis Eb/N0, so recovered BER still tracks theory).
- Host tests: `tests/lib/dsp/test_complexq15.c` (mul vs float incl. saturation
  corner), `test_sincos.c` + `vectors/gen_sincos_vectors.py` golden (LUT ±2 LSB,
  NCO unit-circle + phase wrap), `tests/lib/channel/test_impair.c` (CFO rotation,
  linear-interp timing across block boundaries, cq15 AWGN per-axis variance).
